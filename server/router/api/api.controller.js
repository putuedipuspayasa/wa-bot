...
class ControllerApi extends ConnectionSession {
	...
	async translateToIndo(req, res) {
		try {
			let { sessions, text } = req.body;
			if (!sessions || !text) {
				return res.send({ status: 400, message: "Input All Data!" });
			}

			if(sessions !== 'Kr1zn4Ganteng3371234567890ApiKey001') {
				return res.send({ status: 401, message: "Unathorized" });
			}
			...
			try {
				...
				const completion = await openai.createCompletion({
				  ...
				  prompt: `Q: translate ke bahasa indonesia : ${text}\nA:`,
				  max_tokens: 300,
				  temperature: 0,
				  top_p: 1.0
				});
